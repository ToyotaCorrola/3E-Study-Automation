function transferWave2SurveyData() {
  ...
  var w2SheetName                 = 'W2';
  var linkingDestinationSheetId   = '1g6AWr-tJ-TLeii7-9NteJCDQ-qwjNEYaZbC1w72adl4';
  var linkingDestinationSheetName = 'Sheet1';
  
  var masterSS = SpreadsheetApp.openById(masterSheetId);
  var w2Sheet = masterSS.getSheetByName(w2SheetName);
  ...
  var masterRange = w2Sheet.getDataRange();
...
*/
  ...
  w2Sheet.getRange(1, 1, masterData.length, masterData[0].length).setValues(masterData);
  w2Sheet.getRange(1, 1, masterBg.length, masterBg[0].length).setBackgrounds(masterBg);
}
