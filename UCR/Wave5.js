function transferWave5SurveyData() {
  ...
  var w5SheetName                 = 'W5';
  var linkingDestinationSheetId   = '15ACgCI9mqkxW7YH_oZJSJmYSUrCT2cGNzgMNVrhCgYc';
  var linkingDestinationSheetName = 'Sheet1';
  
  var masterSS = SpreadsheetApp.openById(masterSheetId);
  var w5Sheet = masterSS.getSheetByName(w5SheetName);
  ...
  var masterRange = w5Sheet.getDataRange();
  ...
  var preferredContactMethodIdx   = 20;
  var notesIdx                    = 21;
...
*/
      ...
      if (!mRow[surveyCompletedIdx] || mRow[surveyCompletedIdx].toLowerCase() === "no") { 
  ...
  w5Sheet.getRange(1, 1, masterData.length, masterData[0].length).setValues(masterData);
  w5Sheet.getRange(1, 1, masterBg.length, masterBg[0].length).setBackgrounds(masterBg);
}
