function add_Health_Visit_Study_ID_If_Missing(){
  ...
  for (var i = 1; i < destData.length; i++) {
    ...
    if (studyId.length > 0){
      if(!nameEmail){
        ...
        if (idx !== undefined) {
          let mRow = master.data[idx];
          const MasterEmail = mRow[3];
          nameEmail = MasterEmail;
          //console.log(MasterEmail + " + " + i);
          linkingSheet.getRange(i+1, 5).setValue(nameEmail);
        }
      }
    }
    if (studyId.length === 0) {
      if (nameEmail) {
        ...
        if (idx !== undefined) {
          let mRow = master.data[idx];
          const MasterStudyID = mRow[0]; 
          studyId = MasterStudyID;
          //console.log(nameEmail + " + " + i);
          linkingSheet.getRange(i+1, 1).setValue(studyId);
        }
      }
    }
    if (idx === undefined) 
